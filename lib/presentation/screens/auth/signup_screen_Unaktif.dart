...
class _SignUpScreenState extends ConsumerState<SignUpScreen> {
  ...
  Future<void> _signUp() async {
    ...
    try {
      ...
      // 8. Jika daftar berhasil, panggil 'otak' User (Riverpod) dengan role default
      // NOTE: File ini tidak digunakan dalam sistem admin-only
      if (userCredential.user != null) {
        await ref.read(userRepositoryProvider).saveUserData(
              userCredential.user!.uid,
              _emailController.text.trim(),
              _mobileNumberController.text.trim(),
              UserRole.wali, // Default role (file ini tidak digunakan)
            );
      }
